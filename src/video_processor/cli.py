...
def process(
    ...
    theme: str = typer.Option(
        ..., "--theme", help="Theme name for thumbnail background (looks for thumbnails/<theme>.jpg or .png)"
    ),
    title: str | None = typer.Option(
        None, "--title", help="Custom title for the thumbnail (auto-generated from transcription if not provided)"
    ),
    subtitle: str | None = typer.Option(
        None, "--subtitle", help="Subtitle for the thumbnail (uses description if not provided)"
    ),
    start_from: str | None = typer.Option(
        None, "--start-from", help="Start timestamp in mm:ss format (e.g., '00:03' to start at 3 seconds)"
    ),
    end_at: str | None = typer.Option(
        None, "--end-at", help="End timestamp in mm:ss format (e.g., '05:30' to end at 5 minutes 30 seconds)"
    ),
    thumbnail_duration: float = typer.Option(
        5.0, "--thumbnail-duration", help="Duration to show the thumbnail at the start (seconds)"
    ),
    skip_transcription: bool = typer.Option(
        False, "--skip-transcription", help="Skip transcription and use provided title/subtitle instead"
    ),
    lang: str = typer.Option(
        "nl", "--lang", help="Language for transcription and metadata ('nl' for Dutch, 'en' for English)"
    ),
    ...
    try:
        ...
        console.print("[dim]Create a thumbnails folder and add theme images (e.g., thumbnails/dark.jpg)[/dim]")
        raise typer.Exit(code=1)
...
@app.command()
def info(
    video_name: str = typer.Argument(..., help="Name of the video file in the input folder"),
):
